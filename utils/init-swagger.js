...
exports.addSwaggerPath = (path, method, info, consumes, produces, queryParams = [], bodyParams = {}, moduleName, auth, responses = {}) => {
    ...
    [path]: {
      ...
      [method]: {
        ...
        ...(auth
          ...
          : {}),
        responses: Object.keys(responses).reduce((acc, key) => {
          acc[Responses.HTTP_CODES[key]] = { description: responses[key] };
          return acc;
        }, {})
      }
    }
  ...
  const schema = Object.keys(obj).reduce((prev, key) => {
    if (obj[key].isJoi) {
      prev.push({
        ...
        type: obj[key]._type,
        in: 'query',
        required: obj[key]._flags.presence === 'required' ? true : false
      });
    }
    return prev;
  }, []);
